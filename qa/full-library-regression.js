const fs=require('fs'),vm=require('vm');const {TextDecoder,TextEncoder}=require('util');
...
const ctx={BENTO_RECIPE_LIBRARY:[],console:{log(){},warn(){},error(){}},TextDecoder,TextEncoder,atob:s=>Buffer.from(s,'base64').toString('binary'),btoa:s=>Buffer.from(s,'binary').toString('base64')};ctx.window=ctx;vm.createContext(ctx);
...
const lib=ctx.BENTO_RECIPE_LIBRARY||[],ids=new Map(),titlesByCuisine=new Map();
...
const infer=ctx.BENTO_INFER_ALLERGENS;if(typeof infer!=='function')fail('allergen-runtime-missing');else for(const r of standard){const inf=infer(r.ingredients)||[],stored=new Set((r.allergens||[]).map(x=>String(x).toLowerCase()));for(const a of inf)if(!stored.has(a))fail('post-runtime-allergen-missing',`${r.id}:${a}`)}
