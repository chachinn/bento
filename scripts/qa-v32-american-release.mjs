...
assert(manifest.recipeCount===2297,'manifest total');
assert(manifest.standardRecipeCount===1826,'manifest standard total');
...
assert(rows.length===2297,`full library count ${rows.length}`);
assert(new Set(rows.map(r=>String(r.id))).size===2297,'full library unique IDs');
const counts={};for(const r of rows){const k=String(r.cuisine||'');counts[k]=(counts[k]||0)+1;}
const expected={Japanese:189,Filipino:184,Korean:174,Chinese:90,Thai:90,Vietnamese:130,Indian:171,Italian:165,French:175,Spanish:215,American:243};
...
assert(rows.filter(r=>verified.has(r.cuisine)).length===1826,'standard cuisine runtime count');
...
console.log(JSON.stringify({total:rows.length,uniqueIds:new Set(rows.map(r=>r.id)).size,standard:rows.filter(r=>verified.has(r.cuisine)).length,american:americans.length,americanModules:modules.length,americanStepRange:[5,10],asset:'32p1',release:'32.0'},null,2));
