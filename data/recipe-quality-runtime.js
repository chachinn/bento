...
function inferAllergens(ingredients){
  ...
  const dairy=raw.replace(/\b(coconut|soy|almond|oat|rice|cashew|macadamia) milk\b/g,' plantmilk ');
  ...
  if(has(raw,/\b(fish sauce|bonito|katsuobushi|dashi|anchov(?:y|ies)|salmon|tuna|mackerel|cod|sardines?|fish cake|chikuwa|kamaboko|tilapia|bangus|milkfish|sea bass|perch|trout|snapper)\b/))out.push('fish');
  ...
  if(has(raw,/\b(peanut|peanuts|cashew|cashews|walnut|walnuts|almond|almonds|hazelnut|hazelnuts|pistachio|pistachios|pine nut|pine nuts|chestnut|chestnuts|macadamia)\b/))out.push('nuts');
...
for(const r of lib){
  ...
  r.recipeQualityVersion=27;
}
window.BENTO_RECIPE_QUALITY_VERSION=27;
