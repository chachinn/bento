...
for(const file of files){
  ...
  for(let i=0;i<matches.length;i++){
    ...
    const end=i+1<matches.length?matches[i+1].index:Math.min(text.length,start+12000);
...
if(dupes.length||missing.length||unique.some(r=>!r.title||!r.region||!r.category)) process.exitCode=1;
