...
const cleanBaseline=manifest.recipeCount===2181&&manifest.standardRecipeCount===1710&&manifest.japaneseRecipeCount===252&&manifest.filipinoRecipeCount===248&&manifest.appRelease==='31.3';
const stagedBaseline=manifest.recipeCount===2424&&manifest.standardRecipeCount===1953&&manifest.japaneseRecipeCount===252&&manifest.filipinoRecipeCount===248&&manifest.americanRecipeCount===243&&manifest.appRelease==='32.0';
if(!cleanBaseline&&!stagedBaseline){
  throw new Error(`Unexpected release state: ${JSON.stringify({recipeCount:manifest.recipeCount,standardRecipeCount:manifest.standardRecipeCount,japaneseRecipeCount:manifest.japaneseRecipeCount,filipinoRecipeCount:manifest.filipinoRecipeCount,americanRecipeCount:manifest.americanRecipeCount,appRelease:manifest.appRelease})}`);
}
...
if((index.match(/data\/american-runtime\.js\?v=32p1/g)||[]).length!==1)throw new Error('American runtime index multiplicity');
...
if((sw.match(/\.\/data\/american-runtime\.js\?v=32p1/g)||[]).length!==1)throw new Error('American runtime SW multiplicity');
...
Object.assign(manifest.qualityAuditSnapshot.standardCuisines,{
  recipeCount:STANDARD,Japanese:252,Filipino:248,American:AMERICAN_COUNT,flaggedRecipes:0,allergenFailures:0,referenceFailures:0,equipmentFailures:0,photoMetadataFailures:0,
