...
function sandbox(initial=[]){
  const noop=()=>{};
  const document={querySelector:()=>null,querySelectorAll:()=>[],getElementById:()=>null,createElement:()=>({setAttribute:noop,addEventListener:noop,removeEventListener:noop,style:{}}),addEventListener:noop,removeEventListener:noop};
  const g={
    BENTO_RECIPE_LIBRARY:initial,document,console,setTimeout,clearTimeout,URL,Map,Set,Date,Math,JSON,Array,Object,String,Number,Boolean,RegExp,
...
const photoNorm=s=>String(s||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/[^a-z0-9]+/g,' ').trim();
const photoStop=new Set(['de','del','la','las','el','los','con','en','al','a','y','e','i','o','u','d','l']);
...
for(const r of spanish){
  ...
  const photoOk=Array.isArray(r.photoQueries)&&r.photoQueries.length>=2&&r.photoQueries.every(q=>typeof q==='string'&&q.trim().length>=6)&&new Set(r.photoQueries.map(photoNorm)).size>=2;
  const titleTokens=photoNorm(r.title).split(' ').filter(t=>t.length>=3&&!photoStop.has(t));
  const queryText=photoNorm((r.photoQueries||[]).join(' '));
  assert(photoOk&&(!titleTokens.length||titleTokens.some(t=>queryText.includes(t))),`${r.id}: dish-specific photo queries`);
...
assert(!scriptRefs.some(x=>x.includes('spanish-v31-quality-fixes')),'Temporary Spanish quality-fix script leaked into index');
...
assert(!sw.includes('spanish-v31-quality-fixes'),'Temporary Spanish quality-fix script leaked into service worker');
...
// Execute all production data scripts in browser-like index order, excluding app.js.
...
const standardNames=new Set(Object.keys(expectedCounts));
const nonStandard=lib.filter(r=>!standardNames.has(r.cuisine));
assert(nonStandard.length===471,`Game/anime record count ${nonStandard.length} != 471`);
...
assert(manifest.gameLibraries?.['Genshin Impact']?.recipeRecords===372,'Manifest Genshin record count');
assert(manifest.animeRecipeCount===99,'Manifest Anime count');
...
console.log(JSON.stringify({Spanish:spanish.length,modules:spanishModules.length,total:lib.length,uniqueIds:new Set(lib.map(r=>String(r.id))).size,nonStandard:nonStandard.length,stepDistribution:distribution},null,2));
